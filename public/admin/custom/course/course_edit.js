...
var myDropzone = new Dropzone("div#dropzoneDragArea", {
    ...
    init: function (file) {
        ...
        $('#edit_course_form').on("submit", function (e) {
            ...
            if (myDropzone.getQueuedFiles().length > 0)
            {
                myDropzone.processQueue();
                    ...
                    $('button[type=submit]', '#edit_course_form').html(submit_btn_after);
                    $('button[type=submit]', '#edit_course_form').addClass('disabled');
                    ...
                    $.ajax({
                        ...
                        error: function (err) {
                            $('button[type=submit]', '#edit_course_form').html(submit_btn_before);
                            $('button[type=submit]', '#edit_course_form').removeClass('disabled');
                            if(err.status===403){
                                ...
                                swal({
                                    ...
                                    $('button[type=button]', '##edit_course_form').click();
                                });
                
                            }
                
                            $('#edit_course_form .err-mgs').each(function(id,val){
                            ...
                            $.each(err.responseJSON.errors,function(idx,val){
                                // console.log('#edit_course_form #'+idx);
                                var exp = idx.replace('.','_');
                                var exp2 = exp.replace('_0','');
                                
                                $('#edit_course_form #'+exp).addClass('border-danger is-invalid')
                                $('#edit_course_form #'+exp2).addClass('border-danger is-invalid')
                                $('#edit_course_form #'+exp).next('span').find('.select2-selection--single').attr('id','invalid-selec2')
                                $('#edit_course_form #'+exp).next('.err-mgs').empty().append(val);
                
                                $('#edit_course_form #'+exp+"_err").empty().append(val);
                            })
                        },
                    });
            }
        ...
        this.on('sending', function (file, xhr, formData) {
            ...
            var data = $("#edit_course_form").serializeArray();
            // ardata = [];
            
            // if($('#edit_course_form #course_type').val()=='Pre-recorded'){
    ...
    error: function (file, err) {
        ...
        $('#edit_course_form').find(" :input").each(function(){
            $(this).removeClass('border-danger is-invalid')
        })
        $('#edit_course_form .text-danger').each(function(id,val){
            $(this).empty();
        })

        $.each(err.errors,function(idx,val){
            $('#edit_course_form #'+idx).addClass('border-danger is-invalid')
            $('#edit_course_form .err-mgs-'+idx).empty().append(val);
        })
    },
