        ...
        $.ajax({
            ...
            success: function (data) {
               ...
               $.each(data.langs,function(key,val){
                    $('#add_category_form #category_name_'+val.lang).val(data.tdata[key]);
               })
            },
            error: function (err) {
                if(err.status===403){
...
$(document).on('change','#edit_category_form #translate_autometic',function(){
    if($(this).is(':checked') && $('#edit_category_form #category_name').val()){
        $.ajax({
            type: "get",
            url: translate_url,
            dataType: 'JSON',
            data : {'tdata':$('#edit_category_form #category_name').val()},
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            success: function (data) {
               ...
               $.each(data.langs,function(key,val){
                    $('#edit_category_form #category_name_'+val.lang).val(data.tdata[key]);
               })
            },
            error: function (err) {
                if(err.status===403){
    ...
    $.ajax({
        ...
        success: function (rdata) {
            ...
            swal({
                ...
                let cat_image = data.category_image?'<img style="height: 50px;width:50px;" src="'+base_url+'/'+data.category_image+'">':no_file;
                $('#basic-1 tbody').append(`<tr id="trid-${data.id}" data-id="${data.id}"><td>${data.category_name}</td><td>${cat_image}</td><td>${data.admin.name}</td>
        ...
        success: function (data) {
            ...
            $.each(data.translations,function(key,val){
                if(val.locale=='en'){
                    ...
                    $('#edit_category_form #category_name_'+val.locale).val(val.value);
                }
            })
            ...
            if(data.category_image==''){
                ...
                $('#edit_category_form #image_preview').empty().append(`<img src="${base_url+'/'+data.category_image}">`);
            }
        },
        error: function (err) {
            if(err.status===403){
...
$('#edit_category_form').submit(function (e) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('td:nth-child(2)',trid).html(data.category.category_image?`<img style="height: 50px;width:50px;" src="${base_url+'/'+data.category.category_image}">`:no_file);
            $('td:nth-child(3)',trid).html(data.category.admin.name);
            swal({
                icon: "success",
                title: data.title,
                text: data.text,
