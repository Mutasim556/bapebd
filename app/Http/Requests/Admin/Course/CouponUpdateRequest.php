...
class CouponUpdateRequest extends FormRequest
{
    ...
    {
        return [
            'coupon_code'=>'required|unique:course_coupons,coupon,'.$this->coupon_id,
    ...
    public function update(string $id){
        ...
        $coupon->save();
        $delete = CourseAppliedCoupon::where([['coupon_id',$coupon->id]])->delete();
        if($this->course){
            foreach($this->course as $key=>$val){
                ...
                $coupon_course->updated_by = LoggedAdmin()->id;
                $coupon_course->save();
            }
        }
